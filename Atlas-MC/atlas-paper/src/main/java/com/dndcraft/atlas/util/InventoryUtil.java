...
import com.dndcraft.atlas.menu.icon.Slot;
...
public class InventoryUtil {
	...
	public static int countInventory(Inventory inv, ItemStack is) {
		...
		for (ItemStack other : inv.getContents()) {
			...
			if (other.isSimilar(is) || similarHeads(is, other)) {
	...
	// ItemStack.isSimilar doesn't detect textured heads.
	private static boolean similarHeads(ItemStack itemOne, ItemStack itemTwo) {

		// Material Check
		if (itemOne.getType().equals(Material.PLAYER_HEAD) && itemTwo.getType().equals(Material.PLAYER_HEAD)) {
			ItemMeta metaOne = itemOne.getItemMeta();
			ItemMeta metaTwo = itemTwo.getItemMeta();

			if (metaOne != null && metaTwo != null && metaOne.getDisplayName().equals(metaTwo.getDisplayName())) {
				if (metaOne.hasLore() && metaTwo.hasLore() && !metaOne.getLore().equals(metaTwo.getLore()) ) {
					return false;
				}
				if (!((SkullMeta) metaOne).getOwningPlayer().equals(((SkullMeta) metaTwo).getOwningPlayer())) {
					return false;
				}
				return true;
			}
	...
	public static boolean isSimilar(ItemStack firstItem, ItemStack secondItem) {
		ItemStack copyFirst = firstItem.clone();
		ItemStack copySecond = secondItem.clone();
		copyFirst.setAmount(1);
		copySecond.setAmount(1);

		return serializeItems(copyFirst).equals(serializeItems(copySecond));
	}
	...
	public static List<ItemStack> deserializeItems(String listOfItems) {
		...
		try {
			...
			//	Tythan.get().getLogger().warning("Unable to deserialize the given list;\n" + listOfItems);
			//}
	 ...
	 * See also: {@link Inventory#addItem(ItemStack...)}
		...
		try {
			...
			if(e instanceof InventoryClickEvent) {
				...
				case DROP_ALL_SLOT: case DROP_ONE_SLOT:
					...
					break;
				case HOTBAR_MOVE_AND_READD: /*
					This is chosen instead of HOTBAR_SWAP if:
				...
				case MOVE_TO_OTHER_INVENTORY:
					is = ev.getCurrentItem().clone();
					int destination = ev.getClickedInventory() == e.getView().getTopInventory() ? 
							e.getView().getTopInventory().getSize() + getShiftClickDestination(ev.getView().getBottomInventory()) : getShiftClickDestination(ev.getView().getTopInventory());
					result.add(new MovedItem(is, ev.getRawSlot(), destination));
	...
	private static int getShiftClickDestination(Inventory inv) {
		if (inv.getType() == InventoryType.PLAYER) {
			for (int i = 35 ; i >= 8 ; i--) //go backwards since that is how minecraft does it
			...
			if(holder instanceof Menu) {
				...
				MenuAgent ma = menu.getAgent(inv.getViewers().get(0));
				Validate.notNull(ma);
				for (int i = 0 ; i < menu.getSize() ; i++) {
					Icon icon = menu.getIcon(i);
					if (icon instanceof Slot && icon.getItemStack(ma) == null)
						return i;
				}
				return -1;
			} else {
	...
	private static void handleMoveToOther(List<MovedItem> result, int raw, InventoryView view) {
		...
		case SHULKER_BOX:
			...
			if(!topToBottom && view.getTopInventory().getHolder() instanceof Animals) {
				...
				for(int i = 0; i < 2; i++) {
					if(view.getItem(i).getType() == Material.AIR) {
